...
export function openReader(article, onClose = () => {}) {
  ...
  dialog.setAttribute('aria-label', 'RSVP Reader');
  // This template is static. Article strings are assigned only via textContent.
  dialog.innerHTML = `<div class="frame">
    <header class="chrome"><div class="brand"><span class="mark" aria-hidden="true">r</span> RSVP Reader</div><div class="header-actions"><button class="quiet" id="theme" aria-label="Switch to dark theme">◐</button><button id="close">Close <span aria-hidden="true">↗</span></button></div></header>
    <section class="article chrome"><span class="eyebrow">A little less noise. A little more focus.</span><h1></h1><p id="meta"></p></section>
    ...
    <footer class="chrome"><span>YOUR HAND CONTROLS THE PACE</span><span>One word. Right here.</span></footer>
  </div>`;
